...
public class DNSMessage {
    ...
    // reference on which part of header correspods to which part of buffer array
...
//        hi TA, all the following is from when I tried to implement variables to hold the values. Turns out,
//        I didn't need to do that at all. However, I felt really bad if I deleted it all, which is why it's all
//        commented out here.  Please just ignore the commented code here.

        // qr code - 1 bit - query 0, response 1
//        byte qrop = 0x00;
//        // op code - 4 bit
//        qrop += getOpcode() & 0xFF >> 12; // convert from int to 4 bit
//        // aa - 1 bit - false bc only meaningful in response
//        // qrop += 0x00 >> 3; // useless, for show
//        // tc - 1 bit - not truncated message
//        // qrop += 0x00 >> 2; // useless, for show
//        // rd - 1 bit - recursion desired
//        qrop += 0x01;
//        buffer.put(qrop);// qr, op code, aa, tc, rd
//        // ra - 1 bit - recursion available
//        // z - 3 bit - set to 0, reserved
//        byte raz = 0x10;
//        // rcode - 4 bit
//        raz += getRcode() & 0xFF >> 12;
//        buffer.put(raz);
//        buffer.putShort((short) getQDCount()); // qd - 16 bit
//        buffer.putShort((short) getANCount()); // an - 16 bit - should be 0
//        buffer.putShort((short) getNSCount()); // ns - 16 bit - should be 0
//        buffer.putShort((short) getARCount()); // ar - 16 bit - should be 0
    ...
    public DNSMessage(byte[] recvd, int length) {
        ...
        // TODO: Complete this method
        buffer.position(12);
//        Hi TA this is yet another block of code I couldn't bring myself to delete, please ignore

//        if (length > 0 && (recvd.length > 0)) {
//            this.id = ((recvd[0] & 0xFFFF) + recvd[1]); // id is 16 bits aka 2 bytes
//            // qr is 1 bit, query 1, response 0
//            this.qr = (recvd[2] & 0x80) != 0;
//            this.qr = getQR();
//            // opcode is 4 bits, standard query 0
//            this.opcode = (recvd[2] & 0x78) >> 3;
//            this.aa = (recvd[2] & 0x04) != 0;
//            this.tc = (recvd[2] & 0x02) != 0;
//            this.rd = (recvd[2] & 0x01) != 0;
//            this.ra = (recvd[3] & 0x80) != 0;
//            this.rcode = recvd[3] & 0x0F;
//            this.qdcount = ((recvd[4] & 0xFF) << 8) + recvd[5];
//            this.ancount = ((recvd[6] & 0xFF) << 8) + recvd[7];
//            this.nscount = ((recvd[8] & 0xFF) << 8) + recvd[9];
//            this.arcount = ((recvd[10] & 0xFF) << 8) + recvd[11];
//        }
    }
